...
store?.readFromFile('./vanz_store.json')
// save every 10s
setInterval(() => {
	store?.writeToFile('./vanz_store.json')
}, 10_000)
...
if(usePairingCode && !conn.authState.creds.registered) {
		...
		let phoneNumber = ''
		do {
			phoneNumber = await question(chalk.blueBright('Masukkan Nomor Telepon Kamu diawali 62xxxx:\n'))
...
global.reloadHandler = async function (restatConn) {
  ...
  conn.welcome = 'Selamat Datang Di @subject\nHalo 👋 @user\n\nDan Jangan Lupa Baca Deskripsi Ya😊\n@desc'
  conn.bye = 'Selamat Tinggal @user\nKami Harap Kamu Akan Kembali Lagi'
  conn.spromote = 'Selamat Kamu Menjadi Admin!! @user'
  conn.sdemote = 'Maaf Kamu Tidak Lagi Menjadi Admin!! @user'
